-- ==========================================================
-- TriFit 부품 DB 전체 재구축 (rebuild_full.sql)
-- 편집 중 신규/기존 내용 충돌로 데이터 유실 -> 테이블 자체를 새로 만듦
-- 기존 full_seed.sql 데이터(37 mb / 43 gpu / 28 psu / 30 case / 27 cooler 등,
-- 손상 없는 최신 병합본 확인됨) + 신규 검증된 ASUS ROG x Hatsune Miku 5종 포함
-- ==========================================================

drop table if exists cpu, motherboard, ram, gpu, psu, pc_case, ssd, cooler cascade;
...
-- ============ ASUS ROG x Hatsune Miku Edition 추가분 (5종) ============
-- 기존 mb-37 / gpu-43 / psu-28 / case-30 / cooler-aio-14 다음 번호로 부여 (충돌 없음, 확인됨)
-- 아래 5건은 ROG 공식 스펙 페이지(rog.asus.com) + Newegg/Amazon/Micro Center 판매 페이지로
...
INSERT INTO pc_case (id, model, tower_type, supported_mb, gpu_max_length_mm, cpu_cooler_max_height_mm, psu_support, psu_position, psu_max_length_mm, radiator_top_mm, radiator_front_mm, radiator_side_mm)
VALUES ('case-31', 'ASUS ROG Strix Helios II 하츠네 미쿠 에디션', '빅타워', 'E-ATX;ATX;M-ATX;Mini-ITX', 450, 190, '표준-ATX', '하단후면', '220', '360', '420', '불가')
ON CONFLICT (id) DO UPDATE SET
...
-- 쿨러 (AIO) -- supported_sockets 정정: 초안엔 LGA1200/LGA115x 포함됐으나 공식/Newegg 확인 결과
-- AM4/AM5, LGA1700/LGA1851만 지원 (레거시 소켓 미지원 -- "추정치" 표기 삭제, 확정값으로 교체)
INSERT INTO cooler (id, model, type, height_mm, radiator_size_mm, supported_sockets, extra)
VALUES ('cooler-aio-15', 'ASUS ROG Ryuo IV 360 ARGB 하츠네 미쿠 에디션', 'aqua', null, 360,
  'AM5;AM4;LGA1851;LGA1700',
  '{"display": "6.67인치 곡면 AMOLED", "fan_max_rpm": 2650, "airflow_cfm": 71.44, "pressure_mmh2o": 5.45, "note": "supported_sockets는 ROG 공식 스펙 페이지 + Newegg 판매 페이지 재검증 완료(AM4/AM5, LGA1700/LGA1851만 지원, 레거시 소켓 미지원)", "edition": "hatsune_miku", "source": "rog.asus.com,newegg,dwcts"}'::jsonb)
ON CONFLICT (id) DO UPDATE SET
...
-- ============ 추가 요청분: PCIe5.0 SSD / Intel 칩셋 메인보드 / DDR4 RAM ============
-- 전부 웹서치로 실존 제품 확인 후 추가 (모델명 조작 없음, 불확실한 수치는 note에 명시)

-- ---- SSD: PCIe5.0 신규 5종 (ssd-45 ~ ssd-49) ----
INSERT INTO ssd (id, model, type, interface) VALUES ('ssd-45', 'Micron Crucial T705 M.2 NVMe (1TB)', 'M.2 NVMe', 'PCIe5.0x4')
ON CONFLICT (id) DO UPDATE SET
  model = EXCLUDED.model, type = EXCLUDED.type, interface = EXCLUDED.interface;

INSERT INTO ssd (id, model, type, interface) VALUES ('ssd-46', 'Micron Crucial T710 M.2 NVMe (2TB)', 'M.2 NVMe', 'PCIe5.0x4')
ON CONFLICT (id) DO UPDATE SET
  model = EXCLUDED.model, type = EXCLUDED.type, interface = EXCLUDED.interface;

INSERT INTO ssd (id, model, type, interface) VALUES ('ssd-47', 'Kingston FURY Renegade G5 M.2 NVMe (2TB)', 'M.2 NVMe', 'PCIe5.0x4')
ON CONFLICT (id) DO UPDATE SET
  model = EXCLUDED.model, type = EXCLUDED.type, interface = EXCLUDED.interface;

INSERT INTO ssd (id, model, type, interface) VALUES ('ssd-48', 'Western Digital WD_BLACK SN8100 M.2 NVMe (1TB)', 'M.2 NVMe', 'PCIe5.0x4')
ON CONFLICT (id) DO UPDATE SET
  model = EXCLUDED.model, type = EXCLUDED.type, interface = EXCLUDED.interface;

INSERT INTO ssd (id, model, type, interface) VALUES ('ssd-49', 'Corsair MP700 PRO SE M.2 NVMe (2TB)', 'M.2 NVMe', 'PCIe5.0x4')
ON CONFLICT (id) DO UPDATE SET
  model = EXCLUDED.model, type = EXCLUDED.type, interface = EXCLUDED.interface;
